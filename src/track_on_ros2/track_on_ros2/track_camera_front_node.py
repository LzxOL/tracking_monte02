...
class TrackCameraFrontNode(Node):
    ...
    def __init__(self):
        ...
        # 新增：TF转换相关参数
        self.declare_parameter('head_frame', 'link_h2_head')
        self.declare_parameter('base_frame', 'link_t0_base')
        self.declare_parameter('print_base_3d', True)
        ...
        print_base_3d = self.get_parameter('print_base_3d').get_parameter_value().bool_value
        ...
        self.print_base_3d = print_base_3d
    ...
    def _compute_and_log_3d(self, points, visibility):
        """根据深度与内参，计算并打印3D坐标（相机/头部坐标系），并尝试转换到base坐标系打印。"""
        ...
        # 尝试查询 head->base 的 TF（使用最新）
        tf_ok = False
        T = None
        try:
            # 注意：lookup_transform(target, source, time)
            tf: TransformStamped = self.tf_buffer.lookup_transform(self.base_frame, self.head_frame, Time())
            T = tf
            tf_ok = True
        except Exception as e:
            tf_ok = False
            self.get_logger().warn(f"查询TF失败（{self.head_frame}->{self.base_frame}）：{e}")
        ...
        self.get_logger().info("║     关键点3D坐标 (head系) 及 转换到 base 系后的坐标 (单位: 米)     ║")
        self.get_logger().info("╠═══════════════════════════════════════════════════════════╣")
        
        R = None; t = None
        if tf_ok and T is not None:
            q = T.transform.rotation
            t = np.array([T.transform.translation.x, T.transform.translation.y, T.transform.translation.z], dtype=np.float32)
        ...
        for coord in coords_3d:
            ...
            if not coord['valid']:
                ...
                continue
            xh, yh, zh = coord['x'], coord['y'], coord['z']
            self.get_logger().info(
                f"║ [head] 点{coord['id']+1}: X={xh:7.4f} Y={yh:7.4f} Z={zh:7.4f} (m)               ║"
            )
            if tf_ok and R is not None and t is not None:
                p_head = np.array([xh, yh, zh], dtype=np.float32)
                p_base = R @ p_head + t
                self.get_logger().info(
                    f"║ [base] 点{coord['id']+1}: X={p_base[0]:7.4f} Y={p_base[1]:7.4f} Z={p_base[2]:7.4f} (m)       ║"
                )
