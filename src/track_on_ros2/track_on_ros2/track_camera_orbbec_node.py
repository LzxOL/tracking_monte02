#!/usr/bin/env python3
"""
ROS2节点：Orbbec 颜色图像话题关键点跟踪

与 track_camera_node.py 功能一致，但图像输入来自 Orbbec 发布的 ROS2 话题
...
class TrackCameraOrbbecNode(Node):
    """ROS2节点：订阅 Orbbec 颜色图像话题进行关键点跟踪"""

    def __init__(self):
        super().__init__('track_camera_orbbec_node')
        ...
        self.window_name = "Orbbec Camera Tracking - Click points, SPACE start, Q quit"
        self.should_exit = False

        # 订阅 Orbbec 颜色图像
        self.get_logger().info(f'订阅 Orbbec 颜色图像话题: {camera_topic}')
        ...
        self.get_logger().info('Orbbec 跟踪节点已启动')
